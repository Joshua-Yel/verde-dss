...
export default async function ServiceDemandPage() {
  ...
  const averageMape = totalServicesTracked > 0
    ? (svcTable.reduce((acc, curr) => acc + parseFloat(curr.mape), 0) / totalServicesTracked).toFixed(1)
    : "0.0";
        ...
        <div className="xl:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-3 w-full">
          {[
            ...
            },
            {
              ...
              value: `${averageMape}%`,
              desc: parseFloat(averageMape) < 12 ? "Within target range" : "Above target range",
              icon: (
