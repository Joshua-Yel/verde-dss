...
const AIRA_TEST_SUITE = [
  {
    ...
    expectedBehavior:
      "Analyze the available dashboard data before concluding that context is missing; identify the most likely drivers supported by the snapshot and clearly separate observed facts from hypotheses.",
  ...
  },
  {
    ...
    expectedBehavior:
      "Describe the direction and magnitude of change using the provided time-series data and summarize the main trend clearly.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      ...
      "References the available period data",
      "Avoids invented figures",
    ],
  },
  {
    ...
    expectedBehavior:
      "Compare the relevant KPIs directly and explain which metric is higher and by what evidence.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "States which metric is higher",
      "Uses the snapshot as evidence",
  ...
  },
  {
    ...
    expectedBehavior:
      "Use available operational data such as service mix, weekday patterns, inventory, or staffing signals to infer likely causes while clearly labeling uncertainty.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "Offers a reasoned hypothesis grounded in observed patterns",
      "Does not claim certainty without support",
      "Shows some causal reasoning",
    ],
  },
  {
    ...
    expectedBehavior:
      "Recommend actions based on the dashboard evidence and explain why they are relevant.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      ...
      "Bases the suggestion on the data",
      "Avoids generic advice unrelated to the dashboard",
    ],
  },
  {
    ...
    expectedBehavior:
      "Identify the items that need replenishment from the inventory and restock signals, and present them clearly as actionable next steps.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "The answer should identify the items that need reordering",
      "The answer should be grounded in the inventory snapshot",
  ...
  },
  {
    ...
    expectedBehavior:
      "Interpret the forecast in plain language and connect it to the available context without inventing methodology details.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "Explains the meaning of the forecast",
      "Does not fabricate a methodology",
      "Uses the snapshot context",
    ],
  },
  {
    ...
    expectedBehavior:
      "Explain the KPI in simple business terms and connect it to the operational implication.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "Explains the KPI in plain language",
      "Mentions the operational implication",
      "Does not confuse it with a different metric",
    ],
  },
  {
    ...
    expectedBehavior:
      "Use prior conversation context and the current dashboard data to answer a follow-up question coherently without losing the thread.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "Treats the question as a follow-up",
  ...
  },
  {
    ...
    expectedBehavior:
      "Ask for clarification or infer the likely intent from the current context while staying helpful.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "Acknowledges that the question is ambiguous",
      "Suggests a clarification or likely interpretation",
      "Does not pretend to know the intended target",
    ],
  },
  {
    ...
    expectedBehavior:
      "Decline the out-of-scope request briefly and redirect toward business analysis or dashboard support.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "States the request is outside scope",
      "Offers a relevant alternative",
      "Does not answer as if it were a creative writing assistant",
    ],
  },
  {
    ...
    userQuestion:
      "Anong exact na cause ng pagbaba ng revenue na hindi nakalista sa dashboard?",
    expectedBehavior:
      "Avoid inventing unsupported causes and say that the dashboard does not contain enough evidence to confirm the exact cause.",
    expectedAnswerCharacteristics: [
    ...
    passCriteria: [
      "Does not invent a specific cause",
      "Keeps the answer within evidence",
      "Uses cautious but useful language",
    ],
  },
];

function evaluateAiraAnswer(testCase, answer) {
  const text = answer.toLowerCase();
  const questionText = testCase.userQuestion.toLowerCase();
  const hasAnalyticalSignals = ANALYTICAL_REASONING_HINTS.some((hint) =>
    text.includes(hint),
  );
  const isInventoryQuestion =
    /reorder|restock|stock|need to buy|what to reorder|what should i buy/i.test(
      questionText,
    );
  const hasInventoryActionability =
    /(reorder|restock|stock|critical|flagged|items)/i.test(text) &&
    /\d+/.test(text);
  const isOverlyConservative =
    /i do not have enough context|i don't have enough information|i can only report|i can only confirm|i don't have enough data|i do not have the specific business context|i do not have the qualitative business context/i.test(
      text,
    );
  const analyticalReasoningScore =
    isInventoryQuestion && hasInventoryActionability
      ? 0.75
      : isOverlyConservative && !hasAnalyticalSignals
        ...
        : Math.min(
            ...
            0.6 +
              0.1 *
                ANALYTICAL_REASONING_HINTS.filter((hint) => text.includes(hint))
                  .length,
          );
  const groundednessScore =
    isInventoryQuestion && hasInventoryActionability
      ...
      : Math.min(
          1,
          0.6 +
            0.1 * GROUNDING_HINTS.filter((hint) => text.includes(hint)).length,
        );
  const relevanceScore =
    text.includes(questionText) || text.length > 20 ? 0.8 : 0.5;
  const completenessScore = text.length > 80 ? 0.8 : 0.5;
  const naturalnessScore = 0.75;
  const actionabilityScore =
    /recommend|should|consider|try|improve|focus|reorder|restock|stock|items|flagged/i.test(
      text,
    )
      ? 0.8
      : 0.5;
